...
\begin{nota}[title=¿Todavía no tienes el osciloscopio a mano?]
Prueba primero el modo \texttt{--demo}: simula el instrumento con una señal
sintética, sin hardware ni VISA instalado, para practicar el flujo completo
antes del día de laboratorio.
\begin{lstlisting}
python capturar_osciloscopio.py --demo
python capturar_osciloscopio.py --demo --streaming
\end{lstlisting}
La consola avisa explícitamente que los datos son sintéticos, para que
nunca se confundan con una medición real.
\end{nota}
...
\begin{itemize}
  ...
  \item \texttt{--demo}: simula el instrumento con una señal sintética, sin
        hardware ni VISA, para practicar.
...
(arma disparo $\to$ espera $\to$ descarga $\to$ grafica $\to$ repite), más
un panel lateral con estadísticas de la traza actual (mín./máx., pico a
pico, RMS, número de puntos, ventana de tiempo, frecuencia de
actualización). Los ejes no se reencuadran en cada captura --- el rango se
fija con la primera y solo se \emph{expande} si una traza nueva no entra,
para que la ventana se vea estable y no salte de escala todo el tiempo.
...
\subsection{Espectro de frecuencias (\texttt{--fft})}
\begin{lstlisting}
python capturar_osciloscopio.py --streaming --fft
\end{lstlisting}
Agrega un segundo panel con el espectro de amplitud (FFT) de cada captura,
en escala log-log. Útil para diagnosticar \textbf{ruido de alta
frecuencia}: interferencia de fuentes conmutadas, RF, o el propio contenido
espectral del transitorio (p.\,ej. la frecuencia natural de un RLC).

\begin{aviso}
\textbf{Lo que NO sirve para ver: ruido de red (50/60 Hz).} La resolución
en frecuencia es $1/\text{ventana}$ --- con una ventana típica de pocos
cientos de microsegundos, eso son varios \textbf{kHz} por punto, demasiado
grueso para resolver una frecuencia de 50 Hz. Se necesitaría una ventana de
captura mucho más larga (decenas de milisegundos como mínimo), que este
script no controla directamente --- depende de la base de tiempo
configurada en el osciloscopio.
\end{aviso}

% ---- Promediar capturas -----------------------------------------------------
\section{Promediar capturas (\texttt{--promediar})}
\begin{lstlisting}
python capturar_osciloscopio.py --promediar 20 promedio.csv
\end{lstlisting}
Captura $N$ trazas y guarda su \textbf{promedio punto a punto}, con la
desviación estándar como tercera columna (\texttt{incertidumbre[V]}) ---
el ruido aleatorio cae como $1/\sqrt{N}$ al promediar, el resultado
clásico de mediciones repetidas. El resumen en consola incluye la
reducción de ruido esperada para el $N$ elegido.

\begin{nota}[title=No se combina con --streaming]
Son dos modos distintos: uno repite y monitorea, el otro repite y
promedia. Si se piden ambos a la vez, el script se detiene con un mensaje
de error claro en vez de hacer algo ambiguo.
\end{nota}

% ---- Log de estadisticas -----------------------------------------------------
\section{Log de estadísticas (\texttt{--log-estadisticas})}
\begin{lstlisting}
...
\end{lstlisting}
Agrega una fila (\texttt{disparo, timestamp, V\_min, V\_max, V\_pp, V\_rms,
V\_med}) por cada captura al archivo indicado. Si el archivo ya existe,
sigue agregando filas --- es un log que \textbf{crece entre sesiones}, no
se sobrescribe. Con \texttt{--streaming} se agrega una fila por disparo;
sin él, una fila por ejecución del script.
...
sesión?'' --- el script solo registra los números, no decide qué
significan.
...
\begin{itemize}
  ...
  \item \textbf{Varias repeticiones:} usa \texttt{--promediar N} si quieres
        el promedio con incertidumbre. Si en cambio quieres conservar cada
        traza individual sin promediar, guarda cada una con un nombre
